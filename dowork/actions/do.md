...
> **Part of the dowork skill.** Invoked when routing determines the user is adding a request. Creates a `dowork/` folder in your project for request tracking.
...
All request files go in the project's `dowork/` folder:
- `dowork/` at the project root
- Create the folder if it doesn't exist
- `dowork/assets/` for screenshots and files (can be auto-copied from Claude's image cache)

This folder name reflects the **dowork pattern** - requests created by the do action are processed by the work action.
...
- Live in root `dowork/` as **reference documents** (not actionable work items)
- Are **not** picked up by the work action (it only processes `REQ-*.md`)
- Archive to `dowork/archive/` when **all related REQs** are complete
- The `requests` field in the context frontmatter tracks which REQs belong to it

To get the next number, check existing `REQ-*.md` files in `dowork/`, `dowork/working/`, and `dowork/archive/`, then increment from the highest. Context documents use their own numbering sequence.
...
Read all files in `dowork/` folder. For each parsed request:
...
3. **Copy to project assets**: `cp ~/.claude/image-cache/[uuid]/[n].png dowork/assets/REQ-[num]-[slug].png`
4. **Reference in request file**: Point to `dowork/assets/REQ-[num]-[slug].png` in the Assets section
...
- User can manually save to `dowork/assets/` if needed
...
1. Determine the next REQ number (check `dowork/`, `working/`, and `archive/` for highest existing number)
...
Screenshot saved to dowork/assets/REQ-008-button-click-fix.png
```
