...
> **Part of the dowork skill.** Invoked when routing determines the user wants to process the queue. Processes requests from the `dowork/` folder in your project.
...
- **Root `dowork/`**: The queue - pending requests live here (created by the do action)
...
```
pending (in dowork/, set by do action)
...
1. **List** (don't read) `REQ-*.md` filenames in `dowork/` folder
...
1. Create `dowork/working/` folder if it doesn't exist
2. Move the request file from `dowork/` to `dowork/working/`
...
1. **Update the request file frontmatter** (in `dowork/working/`):
```yaml
---
status: completed
claimed_at: 2025-01-26T10:30:00Z
...
```bash
mkdir -p dowork/archive
```

4. **Move the request file to archive**:
```bash
mv dowork/working/REQ-XXX-slug.md dowork/archive/
```
...
1. **Update frontmatter with error** (in `dowork/working/`):
```yaml
---
status: failed
claimed_at: 2025-01-26T10:30:00Z
...
Implements: dowork/archive/REQ-003-dark-mode.md
...
Implements: dowork/archive/{filename}
...
1. **Re-check** root `dowork/` folder for `REQ-*.md` files (fresh check, not cached list)
...
```
□ Step 1: List REQ-*.md files in dowork/, pick first one
□ Step 2: mkdir -p dowork/working && mv dowork/REQ-XXX.md dowork/working/
...
□ Step 7: mkdir -p dowork/archive && mv dowork/working/REQ-XXX.md dowork/archive/
