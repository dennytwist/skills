...
**Actions:**
- **do**: Capture new tasks/requests → writes to dowork/ folder
- **work**: Process pending requests → executes the queue

## CRITICAL: DO NOT EXECUTE TASKS

**When routing to "do" (adding a task):**
- **ONLY** create a REQ file in dowork/
- **NEVER** perform the actual work described in the task
- **NEVER** edit code, configs, or make changes based on the task content
- The task is CAPTURED, not EXECUTED
- Execution happens later when user runs `/dowork run`

This is a queue system. "do" = add to queue. "work" = process queue.
